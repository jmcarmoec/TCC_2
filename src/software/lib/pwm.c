...
void init_PWM(){    
    LPC_SYSCON->SYSAHBCLKCTRL     |= (1<<7); // Habilita clock do Timer 0 de 16 bits (Pag. 30)
      ...
      */
      ...
      LPC_TMR16B0->PR = CLOCK_SYS-1; // Define o prescale que é usado como granularidade do pwm, ou seja, 1 us de precisão no pulso (pag. 344)
      LPC_TMR16B0->MR3 = PERIOD_SERVO_MOTOR_0*1000;//Define o período de 20 ms (pag. 345)
      LPC_TMR16B0->MR0 = DEFAULT_DUTY_CYCLE_SERVO_MOTOR_0*1000; // Define o duty cycle de 1.7 ms (pag. 345)   
